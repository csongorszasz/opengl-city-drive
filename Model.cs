...
namespace projekt
{
    public class Model : IDisposable
    {
        ...
        {
            var scene = _assimp.ImportFile(path, (uint)PostProcessSteps.Triangulate | (uint)PostProcessSteps.FlipUVs | (uint)PostProcessSteps.RemoveComponent);
        ...
        private unsafe List<Texture> LoadMaterialTextures(Material* mat, TextureType type, string typeName)
        {
            ...
            for (uint i = 0; i < textureCount; i++)
            {
                ...
                if (!skip)
                {
                    var texture = new Texture(_gl, Path.Combine(Directory, path), type);
                    texture.Path = path;
        ...
        private unsafe Material loadMaterial(Material* mat) {
        ...
        public (Vector3 min, Vector3 max) CalculateBoundingBox()
        {
            ...
            {
                var (minMesh, maxMesh) = mesh.CalculateBoundingBox();
                min = Vector3.Min(min, minMesh);
                max = Vector3.Max(max, maxMesh);
            }
